...
const AUTHOR = {
  linkedin: "https://www.linkedin.com/in/ayan-pradhan-160471252/",
  github:   "https://github.com/knowaboutayan",
};
  ...
  return (
    <div className="flex flex-col flex-1 bg-[#020817] overflow-hidden">
      ...
      <section className="relative flex flex-col items-center justify-center text-center px-5 pt-20 pb-16 overflow-hidden">
        ...
        {/* Social circles — top right */}
        <div className="absolute top-5 right-5 flex items-center gap-2 z-10">
          <a href={AUTHOR.linkedin} target="_blank" rel="noopener noreferrer"
            title="LinkedIn"
            className="w-9 h-9 rounded-full bg-[#0c1929] border border-[#1e3a58] hover:border-blue-500/50 hover:bg-blue-500/10 text-slate-500 hover:text-blue-400 flex items-center justify-center transition-all">
            <LinkedInIcon />
          </a>
          <a href={AUTHOR.github} target="_blank" rel="noopener noreferrer"
            title="GitHub"
            className="w-9 h-9 rounded-full bg-[#0c1929] border border-[#1e3a58] hover:border-slate-400/40 hover:bg-white/5 text-slate-500 hover:text-white flex items-center justify-center transition-all">
            <GitHubIcon />
          </a>
        </div>
      ...
      <section className="px-5 pb-16 max-w-4xl mx-auto w-full">
        <div className="p-5 bg-[#0c1929]/60 border border-[#1e3a58]/40 rounded-2xl">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {STACK.map(s => (
              <span key={s.label} className={`px-3 py-1.5 rounded-xl text-xs font-semibold border ${s.color}`}>
                {s.label}
              </span>
            ))}
          </div>
